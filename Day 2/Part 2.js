const passwords = `1-7 j: vrfjljjwbsv
1-10 j: jjjjjjjjjjjj
...
const parser = /([\d]+)[-]([\d]+)[ ]([a-z])[:][ ]([a-z]+)[\n]/gm

let match;
let parsedPasswords = [...passwords.matchAll(parser)].map((x) => [Number.parseInt(x[1]), Number.parseInt(x[2]), x[3], x[4]])

console.log(parsedPasswords)

let validCount = 0;

parsedPasswords.forEach((x) => {
	const [pos1, pos2, letter, password] = x;

	if ((password[pos1 - 1] === letter && password[pos2 - 1] !== letter) || (password[pos1 - 1] !== letter && password[pos2 - 1] === letter)) validCount++;
	console.log(password[pos1], password[pos2], letter)
});
